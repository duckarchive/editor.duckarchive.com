...
const processItems = async (items: ImportItem[]): Promise<number> => {
...
const fetchItemsByQueryParams = async (
  qp: URLSearchParams
): Promise<ImportItem[]> => {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    do {
      const items = await fetchItemsByQueryParams(searchParams);
      count = await processItems(items);
      totalCount += count;
      console.log(
